...
class Reflection(Slide):
    def construct(self):
        ...
        takeaway = MathTex("Angle of ", "{incidence }", "= Angle of", "{ reflection}").set_color_by_MathTex_to_color_map({
            "{incidence }": YELLOW,
            "{ reflection}": BLUE})
        self.add(midplane)
        self.add(normal)
...
class SnellBreakDown(Slide):
    def construct(self):
        initial_snell = MathTex('n_{1}\\sin\\theta_{1} = n_{2}\\sin\\theta_{2}')
...
class Eqs(Slide):
    ...
    def construct(self):
        dpdb = MathTex('d\\phi/ d\\beta = 0').shift(2*UP)
        gross = MathTex('0 = 2 - \\frac{\\frac{4}{3}\\cos\\beta}{\\sqrt{(1- \\frac{16}{9}\\sin^{2}\\beta)}')
        gross2 = MathTex('2 = \\frac{\\frac{4}{3}\\cos\\beta}{\\sqrt{(1- \\frac{16}{9}\\sin^{2}\\beta)}')
        squared = MathTex('4 = \\frac{\\frac{16}{9}\\cos^{2}\\beta}{1- \\frac{16}{9}\\sin^{2}\\beta}')
        sub = MathTex('4 = \\frac{\\frac{16}{9}(1-\\sin^{2}\\beta)}{1- \\frac{16}{9}\\sin^{2}\\beta}}')
        nofrac = MathTex('4[\\frac{16}{9}(1- \\sin^{2}\\beta)] = (1-\\frac{16}{9}\\sin^{2}\\beta)')
        ellipses = MathTex('...').shift(1.5*DOWN)
...
# the purpose of putting it here is to make the next few Slides easier
...
class Reprise(Slide, GetIntersections):
    def construct(self):
        ...
        ra_1_label = MathTex('180 -2\\beta').move_to(red_arrow.get_start()).shift(1*RIGHT).scale(0.75).set_color(RED)
        # this name is misleading
        red_angle_2 =  Arc(arc_center = drop.get_center(), start_angle = PI, angle = normal.get_angle(), color = BLUE, radius = 0.5)
        ra_2_label = MathTex('2\\beta').move_to(red_angle_2).shift(0.5*LEFT).scale(0.75).set_color(BLUE)
        big_angle = Arc(arc_center = in_ray.get_end(), start_angle = PI, angle = normal.get_angle(), radius = 1.8, color = BLUE)
        leftmost_label = MathTex('2\\beta').move_to(big_angle).shift(0.3*UP+.8*LEFT).scale(0.75).set_color(BLUE)
        ...
        yellow_label = MathTex('2\\beta - \\phi').move_to(yellow_angle).shift(0.5*UL+0.2*DOWN).scale(0.5).set_color(YELLOW)
        ...
        snell = MathTex("\\sin","({2\\beta - \\phi})", "= \\frac{4}{3} \\sin", "({\\beta})").move_to((-4,0,0)).set_color_by_MathTex_to_color_map({
            "{2\\beta - \\phi}": YELLOW,
            "{\\beta}": RED
            })
        snell2 = MathTex("({2\\beta - \\phi})", "= \\arcsin\\frac{4}{3}", "( \\sin{\\beta})").move_to((-4,-1.2,0)).set_color_by_MathTex_to_color_map({
            "{2\\beta - \\phi}": YELLOW,
            "{\\beta}": RED
            })
    ...
    def stop(self):
        self.wait(0.3)
        self.pause()
class ManyLines(Slide, GetIntersections):
    def construct(self):
...
class ManyLines2(Slide, GetIntersections):
    ...
    def stop(self):
        self.wait(0.3)
        self.pause()
class PinkFloyd(Slide, GetIntersections):
    def construct(self):
        drop = Circle(radius = 2, color = WHITE).shift(1.5*RIGHT)
        COLOR = {
        'RED':1.33,
        ...
        self.next_slide()
        # I tried to find a neater way to do this but it seems impossible since Create takes only one argument
    ...
    def stop(self):
        self.wait(0.3)
        self.pause()
class DoubleRainbow(Slide, GetIntersections):
    def construct(self):
        drop = Circle(radius = 2, color = WHITE).shift(1.5*RIGHT)
        self.add(drop)
        ...
        phi_0_label = MathTex('\\phi').move_to(phi_0).scale(0.6).shift(LEFT*.25 + 0.1*UP)
        phi_1_label = MathTex('\\phi').move_to(phi_1).scale(0.6).shift(LEFT*.25 + 0.1*DOWN)
        normal_list = []
        ...
        red_label = MathTex('90 - \\beta').move_to(red_arrow.get_start()+0.65*RIGHT).scale(0.6).set_color(RED)
        green_label = MathTex('180 - 2\\beta').move_to(green_arrow.get_start()+0.3*UP).scale(0.6).set_color(GREEN)
        purple_label = MathTex('3\\beta - 90').move_to(purple_angle).shift(0.8*LEFT).scale(0.6).set_color(PURPLE)
        purple_label_2 = MathTex('3\\beta - 90').move_to(purple_angle_2).shift(0.8*LEFT).scale(0.6).set_color(PURPLE)
        beta_0_label = MathTex('\\beta').move_to(beta_0).scale(0.5).shift(0.25*RIGHT + 0.05*UP)
        beta_1_label = MathTex('\\beta').move_to(beta_1).scale(0.5).shift(0.25*RIGHT + 0.05*DOWN)
        beta_2_label = MathTex('\\beta').move_to(beta_2).scale(0.5).shift(0.2*DL)
        beta_3_label = MathTex('\\beta').move_to(beta_3).scale(0.5).shift(0.2*LEFT+DOWN*.05)
        # ahh, almost done. Now I will create the last equations:
        snell = MathTex('\\sin(3\\beta - 90^{\\circ} + \\phi) = \\frac{4}{3}\\sin(\\beta)').shift(4*LEFT).scale(0.8)
        phi_eq = MathTex('\\implies \\phi = 90^{\\circ} - 3\\beta + \\arcsin(\\frac{4}{3}\\beta)').shift(1.5*RIGHT).scale(0.8)
        phi_min = MathTex('\\phi_{\\text{min}} \\approx 25.4^{\\circ}').shift(DOWN)
        very_last_phi = MathTex('2\\phi_{\\text{min}} \\approx 51^{\\circ}').shift(DOWN)
        # super dumb but at least it worked:
        very_last_but_bigger = MathTex('2\\phi_{\\text{min}} \\approx 51^{\\circ}').scale(2)
        self.play(Create(horizon))
        self.next_slide()
        self.play(Create(up_horizon), Create(down_horizon))
        self.next_slide()
        self.play(Create(normal_list[1]), Create(normal_list[2]), Create(normal_list[3]), Create(normal_list[4]))
        self.next_slide()
        self.play(Create(phi_0), Create(beta_0), FadeIn(beta_0_label), FadeIn(phi_0_label))
        self.next_slide()
        self.play(Create(phi_1), Create(beta_1), FadeIn(beta_1_label), FadeIn(phi_1_label))
        self.next_slide()
        self.play(Create(beta_2), FadeIn(beta_2_label))
        self.next_slide()
        self.play(Create(beta_3), FadeIn(beta_3_label))
        self.next_slide()
        self.play(FadeIn(perp_bi))
        self.next_slide()
        self.play(Create(red_angle))
        self.next_slide()
        self.play(Create(red_arrow), Write(red_label))
        self.next_slide()
        self.play(Create(green_angle))
        self.next_slide()
        self.play(Create(green_arrow), Write(green_label))
        self.next_slide()
        self.play(Create(purple_angle), Write(purple_label))
        self.next_slide()
    ...
    def stop(self):
        self.wait(0.3)
        self.pause()
class BigPicture(Slide, GetIntersections):
    def construct(self):
        ...
        disclaimer = MathTex('*not to scale').move_to((5, 0, 0)).scale(.7)
        ...
        arc_between = Arc(
            ...
            radius = 3)
        self.play(Create(arc_between))
        label_for_arc = MathTex('42^{\\circ}').move_to(arc_between).shift(LEFT)
